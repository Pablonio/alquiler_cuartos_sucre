...
interface Comentario {
  ...
  calificacion: number;
  usuario: string;
  fechaDeRegistroComentario: Date;
}
  ...
  return (
    <div className="rounded overflow-hidden shadow-lg bg-white text-black m-4 max-w-md w-full sm:max-w-xs sm:w-full md:max-w-sm md:w-1/2 lg:w-1/3 xl:w-1/4">
      <div className="px-6 py-4">
        <div className="font-bold text-xl mb-4">{comentario.usuario}</div>
        ...
        <p className="text-sm text-gray-500">{comentario.fechaDeRegistroComentario.toString()}</p>
      </div>
    </div>
  );
}
